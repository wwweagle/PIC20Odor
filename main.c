...
//int addOdor(intNode* head) {
//    int odor = getFuncNumber(2, "Add an odor");
//    if(head->value<0){
//        head->value=odor;
//    }else{
//        intNode* last=head->next;
//        while(last->next!=NULL){
//            last=last->next;
//        }
//        intNode* next=malloc(sizeof(intNode));
//        next->value=odor;
//        next->next=NULL;
//        last->next=next;
//       
...
int setSessionNum_G2() {

    int d[] = {0, 1, 5, 10, 12, 15, 20, 24, 30};
    unsigned int n = getFuncNumber(1, "1 5 10 12 20 24");
    if (n > 0 && n < sizeof (d))
        return d[n];
    else return 5;
}
...
void stim_G2(int place, int n, int type) {
    ...
    if (place == 3) {
        ...
        switch (place) {
            case 1:
                LCD_Write_Char('1');
                waitTimerJ_G2(taskParam.sample1Length - 200);
                break;
            case 2:
                LCD_Write_Char('2');
                waitTimerJ_G2(taskParam.test1Length - 200);
                break;
            case 4:
                LCD_Write_Char('3');
        ...
        switch (place) {
            case 1:
                ...
                if (taskParam.sample1Length < 1000u)
                    waitTimerJ_G2(1000u - taskParam.sample1Length);
                break;
            case 2:
                assertLaser_G2(type, atSecondOdorEnd);
                LCD_Write_Char('D');
                if (taskParam.test1Length < 1000u)
                    waitTimerJ_G2(1000u - taskParam.test1Length);
                break;
            case 4:
...
static int waterNResult_G2(int firstOdor, int secondOdor, int id) {
    ...
    switch (taskType_G2) {
        ...
        case DNMS_2AFC_TASK:
            //
            ///////////Detect/////////////////
            for (timerCounterI = 0; timerCounterI < 500 && !(lick_G2.portSide == 2 || lick_G2.portSide == 3); lick_G2.portSide = lick_G2.current);
            /////Reward
            if (!lick_G2.portSide) {
                processMiss_G2((firstOdor != secondOdor) ? 2 : 3);
            } else if (!(lick_G2.portSide & 1) != !(firstOdor^secondOdor)) {
                processHit_G2(lick_G2.portSide);
            } else {
                processFalse_G2(lick_G2.portSide == LICKING_LEFT ? 2 : 3);
            }
            break;
            ...
            //            //
            //            ///////////Detect/////////////////
            //            for (timerCounterI = 0; timerCounterI < 500 && !(lick_G2.flag == 2 || lick_G2.flag == 3); lick_G2.flag = lick_G2.current);
            //            /////Reward
            //            if (!lick_G2.flag) {
            //                processMiss_G2((firstOdor != secondOdor) ? 2 : 3);
        ...
        case GONOGO_2AFC_TEACH:
            ...
            /////Reward
            if (lick_G2.portSide == LICKING_LEFT && isLikeOdorA_G2(firstOdor)) {
                processHit_G2(2);
            } else if (lick_G2.portSide == LICKING_RIGHT && !isLikeOdorA_G2(firstOdor)) {
                processHit_G2(3);
            } else if (lick_G2.portSide) {
                processFalse_G2(lick_G2.portSide == LICKING_LEFT ? 2 : 3);
            } else {
...
static void zxLaserTrial_G2(int s1, int t1, int s2, int t2, int laserType) {
        ...
        default:////////////////////////////////////DELAY/////////////////////
            if (taskParam.delay1 == 0) {
                ...
                if (taskParam.delay1 < 4) {
                    waitTimerJ_G2(taskParam.delay1 * 1000u - 2000u);
                } else {

                    //                    waitTimerJ_G2(500u);
                    //                    assertLaser_G2(type, atDelay1_5SecIn); ////////////////1.5Sec////////////////
                    ...
                    //                            || taskType_G2 == DNMS_DUAL_TASK_LEARNING || taskType_G2 == DNMS_DUAL_TASK || taskType_G2 == DUAL_TASK_EVERY_TRIAL) {
                    //                        assertLaser_G2(type, atPreDualTask); //@2s
                    //                        distractor_G2(taskParam.test2, taskParam.sample2, waterLen);
                    //                        waitTimerJ_G2(1500u);
                    //                        assertLaser_G2(type, atPostDualTask); //distractor@3.5sec
                    //                        if (taskParam.delay1 > 8u) {
                    //                            waitTimerJ_G2((taskParam.delay1 - 8u)*500u);
                    //                        }
                    //                    } else 
                    //                    if (taskParam.delay1 >= 12u) {
                    //                        waitTimerJ_G2(500u);
                    //                        assertLaser_G2(type, atDelay2_5SecIn);
                    //                        waitTimerJ_G2(500u);
                    //                        assertLaser_G2(type, atDelay3SecIn);
                    //                        waitTimerJ_G2(500u); //->3.5s
                    //                        assertLaser_G2(type, atDelay3_5SIn);
                    //                        waitTimerJ_G2(1000u); //->4.5s
                    //                        assertLaser_G2(type, atDelay4_5SIn);
                    //                        waitTimerJ_G2(1000u);
                    //                        waitTimerJ_G2((taskParam.delay1 - 12u)*500u);
                    //                    } else 
                    //                    {
                    waitTimerJ_G2(taskParam.delay1 * 500u - 2000u);
                    //                    }
                    assertLaser_G2(laserType, atDelayMiddle); //13@6.5
                    if (taskParam.delay1 >= 12) {
                        waitTimerJ_G2(500u); //13@7
                        assertLaser_G2(laserType, atDelayMid500mSec);
                        waitTimerJ_G2(500u); //13@7.5
                        assertLaser_G2(laserType, atDelayMid1Sec);
                        waitTimerJ_G2(500u); //13@8S
                        assertLaser_G2(laserType, atDelayMid1_5Sec);
                        waitTimerJ_G2(500u); //13@8.5
                        assertLaser_G2(laserType, atDelayMid2Sec);
                        waitTimerJ_G2(500u); //distractor@9s//13@9
                        assertLaser_G2(laserType, atDelayMid2_5Sec);
                        waitTimerJ_G2(500u); //distractor@9.5s//13@9.5
                        assertLaser_G2(laserType, atDelayMid3Sec);
                        waitTimerJ_G2((taskParam.delay1 - 11)*500u); //13@10
                    } else {
                        waitTimerJ_G2(taskParam.delay1 * 500u - 2500u);
                    }
                    assertLaser_G2(laserType, atDelayLast2_5SecBegin);
                    waitTimerJ_G2(500u); //13@10.5
    ...
    int delayLick = waitTimerJ_G2(1000u);
    stim_G2(3, taskParam.respCue, laserType);
    delayLick |= waitTimerJ_G2(500u);
    int resultRtn = 0;
    LCDsetCursor(3, 0);
    if (delayLick) {
        muxDis(0x0f);
    ...
    waitTimerJ_G2(1050u); //water time sync
    // Total Trials
    ...
    if (taskParam.ITI >= 5u) {
        ...
        while (trialITI > 60u) {
            waitTimerJ_G2(60u * 1000u);
            trialITI -= 60u;
        }
        waitTimerJ_G2(trialITI * 1000u); //another 4000 is at the beginning of the trials.
    }
            ...
            for (iterOf4 = 0; iterOf4 < 4 && currentMiss < missLimit; iterOf4++) {
                ...
                switch (taskType_G2) {
                    ...
                    case GONOGO_LR_TASK:
                        ...
                        //                            sample1 = (index == 0 || index == 2) ? taskParam.sample1s[0] : taskParam.sample1s[1];
                        //                            test1 = (index == 1 || index == 2) ? taskParam.sample1 : taskParam.test1;
                        //                            lastOdor1 = sample1;
                        //                            lastOdor2 = test1;
                        //                        } else {
                        //                            sample1 = lastOdor1;
